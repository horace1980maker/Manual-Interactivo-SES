...
import { AmenazaClimatica, AmenazaNoClimatica, PrioritizedLivelihood, AmenazaMedioVidaConflicto } from '../../types';
...
const H3_2_PRIORITIZED_LIVELIHOODS_KEY = 'prioritizedLivelihoods_H32';
const H4_2_1_CHARACTERIZED_CONFLICTS_KEY = 'characterizedMedioVidaConflicts_H421';

type Stage = 'viewList' | 'selectThreat' | 'selectLivelihood' | 'detailConflict';

const initialConflictDetailState: Omit<AmenazaMedioVidaConflicto, 'id' | 'amenaza' | 'medioDeVida' | 'codigoMedioVida'> = {
...
type AmenazaMedioVidaDimensionKey = 'politica' | 'economica' | 'alimentaria' | 'sanitaria' | 'ambiental' | 'personal' | 'comunitaria';

export const Herramienta4_2_1_AmenazasMediosVidaConflictos: React.FC = () => {
  ...
  const [allMediosDeVida, setAllMediosDeVida] = useState<PrioritizedLivelihood[]>([]);
  ...
  const [selectedLivelihood, setSelectedLivelihood] = useState<PrioritizedLivelihood | null>(null);
  ...
  const [characterizedConflicts, setCharacterizedConflicts] = useState<AmenazaMedioVidaConflicto[]>([]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const livelihoodsRaw = sessionStorage.getItem(H3_2_PRIORITIZED_LIVELIHOODS_KEY);
      setAllMediosDeVida(livelihoodsRaw ? JSON.parse(livelihoodsRaw) : []);
      
      const characterizedRaw = sessionStorage.getItem(H4_2_1_CHARACTERIZED_CONFLICTS_KEY);
      setCharacterizedConflicts(characterizedRaw ? JSON.parse(characterizedRaw) : []);

    } catch (error) {
      console.error("Error loading data for H4.2.1:", error);
    }
    setIsLoading(false);
  }, []);
  ...
  const handleThreatSelect = (threat: AmenazaClimatica | AmenazaNoClimatica) => {
    ...
    setCurrentStage('selectLivelihood');
  };

  const handleLivelihoodSelect = (livelihood: PrioritizedLivelihood) => {
  ...
  const handleSaveConflict = () => {
    if (!selectedThreat || !selectedLivelihood) return;
    ...
    const newEntry: AmenazaMedioVidaConflicto = {
      id: `${selectedThreat.id}_${selectedLivelihood.id}_${Date.now()}`,
      amenaza: selectedThreat.nombre,
      medioDeVida: selectedLivelihood.nombreMedioVida,
      codigoMedioVida: selectedLivelihood.codigoMedioVida, // Store the code
      ...conflictDetails,
    ...
    sessionStorage.setItem(H4_2_1_CHARACTERIZED_CONFLICTS_KEY, JSON.stringify(updatedConflicts));
  ...
  const categorizedConflicts = useMemo(() => {
    const climatic: AmenazaMedioVidaConflicto[] = [];
    const nonClimatic: AmenazaMedioVidaConflicto[] = [];
  ...
  if (isLoading) {
    return <ToolCard title="H4.2.1"><p>Cargando datos...</p></ToolCard>;
  }

  if (currentStage === 'selectThreat') {
    return (
      <ToolCard 
        title="H4.2.1 - Paso 1: Seleccionar Amenaza"
  ...
  if (currentStage === 'selectLivelihood' && selectedThreat) {
    return (
      <ToolCard 
        title={`H4.2.1 - Paso 2: Seleccionar Medio de Vida para "${selectedThreat.nombre}"`}
        objetivo="Elija un medio de vida (priorizado en H3.2) que es afectado por la amenaza seleccionada."
      >
        ...
        <h4 className="text-lg font-semibold text-[#001F89] mb-2">Medios de Vida Priorizados (H3.2)</h4>
        {allMediosDeVida.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {allMediosDeVida.map(mdv => <Button key={mdv.id} onClick={() => handleLivelihoodSelect(mdv)} variant="outline" className="text-left justify-start p-3 h-auto">{mdv.nombreMedioVida} ({mdv.codigoMedioVida})</Button>)}
          </div>
        ) : <p className="text-sm text-gray-500">No hay medios de vida priorizados disponibles de H3.2.</p>}
      </ToolCard>
    );
  }
  
  if (currentStage === 'detailConflict' && selectedThreat && selectedLivelihood) {
    const dimensionFields: Array<{key: AmenazaMedioVidaDimensionKey, label: string}> = [
    ...
    return (
      <ToolCard 
        title="H4.2.1 - Paso 3: Detallar Impacto y Conflicto"
        objetivo="Caracterizar cómo la amenaza impacta las dimensiones de seguridad del medio de vida y los conflictos resultantes."
        >
        <div className="mb-6 p-4 bg-[#EBF5FF] border border-[#009EE2]/30 rounded-lg shadow">
            <p className="font-bold text-lg text-[#001F89]">Amenaza: <span className="font-normal text-gray-700">{selectedThreat.nombre}</span></p>
            <p className="font-bold text-lg text-[#001F89]">Medio de Vida: <span className="font-normal text-gray-700">{selectedLivelihood.nombreMedioVida} ({selectedLivelihood.codigoMedioVida})</span></p>
        </div>
        ...
        <div className="md:flex md:space-x-6">
            ...
            <div className="md:w-1/2 space-y-4">
                 <div className="p-3 border rounded-md bg-gray-50">
                    <p className="text-sm font-medium text-gray-700">Medio de Vida (MdV)</p>
                    <p className="text-lg font-semibold text-[#001F89]">{selectedLivelihood.nombreMedioVida}</p>
                 </div>
                 <Input 
                    label="#familias (dependientes del MdV)" 
        ...
        <div className="mt-8 pt-6 border-t border-gray-300">
            <h4 className="text-lg font-semibold text-gray-800 mb-3">La <span className="text-[#001F89] font-bold">{selectedThreat.nombre}</span> ha generado conflictos</h4>
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
            ...
            <Input 
                label="Número de familias específicamente impactadas (por la amenaza directa al MdV):" 
        ...
        <div className="mt-8 flex justify-between">
          <Button onClick={() => setCurrentStage('selectLivelihood')} variant="outline" size="md">
  ...
  const renderConflictList = (list: AmenazaMedioVidaConflicto[], title: string) => (
    list.length > 0 && (
        <>
            ...
            {list.map(item => (
                ...
                <div className="flex justify-between items-start mb-2">
                    <div>
                        ...
                        <p className="text-sm text-gray-600">afectando a <span className="font-medium">{item.medioDeVida}</span></p>
                    </div>
                ...
                <details className="text-xs mt-2">
                    <summary className="cursor-pointer text-indigo-600 hover:text-indigo-800 text-sm">Ver más detalles...</summary>
                    <div className="mt-2 p-3 bg-gray-50 rounded space-y-1">
                        ...
                        <p><strong>Familias (MdV):</strong> {item.numeroFamilias}, <strong>Impactadas (Amenaza):</strong> {item.familiasImpactadas}</p>
  ...
  return (
    <ToolCard 
        title="H4.2.1. CARACTERIZACIÓN DE AMENAZAS (PARA MEDIOS DE VIDA) E IDENTIFICACIÓN DE CONFLICTOS"
        objetivo="Caracterizar cómo las amenazas identificadas impactan los medios de vida y qué conflictos se generan o exacerban."
    >
      <div className="mb-6 flex justify-between items-center">
        <Button onClick={() => navigate('/paso4/herramienta4_1')} variant="outline" size="sm" className="mb-0">
            <ChevronLeftIcon className="h-4 w-4 mr-1 inline-block" />
            Volver a H4.1 (Matrices Amenazas)
        </Button>
      ...
      {characterizedConflicts.length === 0 ? (
        <div className="p-4 border rounded-md bg-blue-50 border-blue-200 text-blue-700 flex items-center">
            ...
            No hay caracterizaciones de amenazas-medios de vida-conflictos añadidas todavía.
        </div>
      ) : (
        <>
       ...
       <div className="mt-10 text-center">
            <Button
              variant="secondary"
              size="lg"
              onClick={() => navigate('/paso4/herramienta4_2_2')}
              aria-label="Proceder a H4.2.2 (Amenazas - Servicios Ecosistémicos - Conflictos)"
            >
              Proceder a H4.2.2 <ArrowRightIcon className="h-5 w-5 ml-2 inline-block" />
            </Button>
        </div>
    </ToolCard>
  );
